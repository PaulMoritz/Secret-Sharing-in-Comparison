...
calc_function(coeff_list, level, x, new_level, prime_number):
...
- level(Integer): The level of the shareholder whose value we calculate
- x (Integer): The x-value of the shareholder (for each shareholder s_level_x)
- new_level(boolean): do we look at a new depth of levels (need to derivate)
- prime_number (Integer): Prime number to build the finite field
