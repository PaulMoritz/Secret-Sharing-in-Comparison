...
def main():
    ...
    with open("tassa_setups.yaml", 'r') as stream:
        ...
        for doc in docs:
            ...
            print("~"*100)
            # setup disjunctive
            delete_setup(disjunctive_name)
            print("\nCalling setup('{}', {}, conjunctive=False):".format(disjunctive_name, levels))
            setup(disjunctive_name, levels, field_size=prime, conjunctive=False)

            # share disjunctive
            print("\nCalling share('{}', {}, prime_number={}, print_statements=False):"
                  .format(disjunctive_name, random_message, prime))
            original_function, original_shares = share(disjunctive_name, random_message,
                                                       field_size=prime, print_statements=True)

            # reconstruct disjunctive
            print("\nCalling reconstruct('{}', number_of_people={}, "
                  "random_subset=True, subset={{}}, print_statements=False):"
                  .format(disjunctive_name, thresholds[-1]))
            try:
                secret, resulting_function, original_determinant, other_determinants, matrix\
                    = reconstruct(disjunctive_name, thresholds[-1], print_statements=True)
            except TypeError as e:
                print("Could not reconstruct to a valid integer-result (test_setups): {}".format(e))
                sys.exit(1)
            if not secret == random_message:
                print("Reconstruction in setup {} calculated an incorrect result (should be {} but was {})"
                      .format(disjunctive_name, random_message, secret))
                sys.exit(1)
            else:
                print("The reconstructed function is\t{}, the message is\t{}\n\t"
                      "(Original function was\t{} with secret\t{})"
