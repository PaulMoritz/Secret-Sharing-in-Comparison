...
def thresholds_fulfilled(setup, person_IDs, print_statements, conjunctive):
    ...
    th.insert(0, 0)
    if conjunctive:
        return thresholds_fulfilled_conjunctive(count_of_persons, person_IDs, print_statements, th)
    else:
        return thresholds_fulfilled_disjunctive(count_of_persons, person_IDs, print_statements, th)


def thresholds_fulfilled_conjunctive(count_of_persons, person_IDs, print_statements, th):
...
def thresholds_fulfilled_disjunctive(count_of_persons, person_IDs, print_statements, th):
    t = th[-1]
    print(th)
    for number_of_level, item in enumerate(th):
        print("item", item)
        persons_in_this_level = 0
        if number_of_level == len(th):
            break
        for person in person_IDs:
            # if person belongs to level, increase count
            if int(person[1]) == t - int(item):
                count_of_persons += 1
                persons_in_this_level += 1
        # after all persons are viewed, if there are less than the threshold, break
        if persons_in_this_level >= th[number_of_level]:
            print("Threshold t_{} fulfilled, authorised subset.".format(number_of_level+1))
            return True
        elif count_of_persons >= th[number_of_level]:
            print("Threshold t_{} fulfilled by filling with people from higher levels".format(number_of_level+1))
            return True
        else:
            if print_statements:
                print("Threshold {} in disjunctive setup not fulfilled, the subset contains only {} people in and "
                      "{} people up to this level. (Should be at least {})"
                      .format(number_of_level+1, persons_in_this_level, count_of_persons, th[number_of_level]))
    # return false only if all thresholds are not fulfilled
    print("fini")
    return False
