...
def derive_function(function_to_derive, prime_number):
    ...
    for level in function_to_derive:
        factor = (level[0] * level[1]) % prime_number
        # catch x^0, don't subtract exponent here
        if level[1] > 0:
            exponent = level[1] - 1
        else:
            exponent = 0
        resulting_function.append([factor, exponent])
...
def calc_function(function_structure, x, prime_number):
    ...
    for coefficient in function_structure:
        result = (result + (int(coefficient[0]) * pow(int(x), int(coefficient[1]))) % prime_number) % prime_number
