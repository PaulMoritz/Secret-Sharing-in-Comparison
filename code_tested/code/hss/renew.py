...
def renew(setup, old_shares, reset_version_number=None, print_statements=True):
    ...
    data, degree_of_function, field_size, number_of_old_shares, thresholds = get_all_data_renew(old_shares,
    ...
    # troubleshooting
    check_and_print_validity(data, field_size, new_shares, print_statements, rec_function, rec_result, shares)
    ...
    compute_and_add_results(field_size, function_dict, new_shares, partial_shares, thresholds)
    ...
    if not (new_result == rec_result):
        ...
        print_renew_results(new_function, new_result, rec_function, resulting_shares)
...
def print_renew_results(new_function, new_result, rec_function, resulting_shares):
...
def compute_and_add_results(field_size, function_dict, new_shares, partial_shares, thresholds):
...
def write_old_share_values(old_shares, partial_shares):
    for ind, value in enumerate(old_shares):
        partial_shares[0][ind] = old_shares[value]


def check_and_print_validity(data, field_size, new_shares, print_statements, rec_function, rec_result, shares):
