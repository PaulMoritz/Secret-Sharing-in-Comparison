...
data_path_sss = get_data_path_sss()


# deletes the folder and all its files if a directory with the given name exists
def delete_setup(name, hierarchical=True):
    if hierarchical:
        file_path = os.path.join(data_path, name)
    else:
        file_path = os.path.join(data_path_sss, name)
    # only if file exists
...
def create_info_file(conjunctive, file_path, lvl_list, name, field_size, hierarchical=True):
    # get creation time
...
# setup("dis", [[1, 1], [2, 3], [4, 7]], conjunctive=False )