...
# double            Indicating if we need double the number of people eg for multiply
def read_shares(setup, double=False):
    data_path = get_data_path_sss()
    if double:
        file_path = os.path.join(data_path, setup, 'shares_after_reset_0.csv')
    else:
